...
module%test _ = struct
  ...
  module%test [@name "Interval.V1.Float"] _ = Stable_unit_test.Make (struct
      include V1.Float
  ...
  module%test [@name "Interval.V1.Int"] _ = Stable_unit_test.Make (struct
      include V1.Int
  ...
  module%test [@name "Interval.V1.Ofday"] _ = Stable_unit_test.Make (struct
      include V1.Ofday
...
module%test [@name "vs array"] _ = struct
  open Interval.Int
  ...
  let%test_unit "mem" =
    Quickcheck.test interval_and_nearby_int ~sexp_of:[%sexp_of: t * int] ~f:(fun (t, i) ->
  ...
  let%expect_test "explicit binary_search" =
    ...
    in
    pr @@ binary_search (create 4 80) ~compare:Int.compare `First_strictly_greater_than 18;
    [%expect {| (19) |}];
    pr
    @@ binary_search (create 25 80) ~compare:Int.compare `First_strictly_greater_than 18;
    [%expect {| (25) |}];
    pr
    @@ binary_search (create 25 80) ~compare:Int.compare `First_strictly_greater_than 1000;
    [%expect {| () |}]
  ;;
end
...
module%test _ = struct
  ...
  let intervals = [ Int.empty; Int.create 3 6; Int.create 2 7; Int.empty; Int.create 4 5 ]
  ...
  let intervals = [ Int.empty; Int.create 3 6; Int.create 2 3; Int.empty; Int.create 4 5 ]
...
module%test _ = struct
  ...
  let%expect_test "Generated intervals sets are very likely both to contain and not \
    ...
    Quickcheck.test_can_generate gen ~trials:10 ~f:(fun (t, x) -> not (Set.contains t x))
  ;;
