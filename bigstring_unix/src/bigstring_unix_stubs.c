  ...
  do {
    ...
    if (n_read <= 0) {
      ...
      if (n_read == 0) {
        ...
        raise_unix_io_error(v_n_good, "read", Nothing);
    } else {
      bstr += n_read;
      len -= n_read;
    }
...
CAMLprim value bigstring_pread_stub(value v_min_len, value v_fd, value v_offset,
  ...
  do {
    do {
      n_read = pread(fd, bstr, len, offset);
    } while (n_read == -1 && errno == EINTR);
    if (n_read <= 0) {
      value v_n_good = Val_long(bstr - bstr_start);
      caml_leave_blocking_section();
...
CAMLprim value bigstring_pread_assume_fd_is_nonblocking_stub(
  ...
  if (n_read == -1)
    uerror("bigstring_pread_assume_fd_is_nonblocking_stub", Nothing);
  return Val_long(n_read);
}
...
CAMLprim value bigstring_recv_peek_assume_fd_is_nonblocking_stub(value v_sock,
  ...
  if (len == 0)
    ...
    if (n_read == -1)
      uerror("bigstring_recv_peek_assume_fd_is_nonblocking", Nothing);
      ...
      if (n_read <= 0) {
        if (n_read == -1) {
          ...
          if (errno == EINTR)
            ...
            value v_n_good = Val_long((char *)bstr_iov->iov_base - bstr_start);
...
CAMLprim value bigstring_output_stub(value v_min_len, value v_chan, value v_pos,
                                     value v_len, value v_bstr) {
  CAMLparam2(v_chan, v_bstr);
  struct channel *chan = Channel(v_chan);
  char *bstr = get_bstr(v_bstr, v_pos);
    ...
    while (1) {
      ...
      if (written == -1) {
        ...
        if ((errno == EAGAIN || errno == EWOULDBLOCK) &&
          ...
          if (buff_len)
            memmove(chan->buff, buff_iov->iov_base, buff_len);
          caml_leave_blocking_section();
          Unlock(chan);
...
CAMLprim value bigstring_write_assume_fd_is_nonblocking_stub(value v_fd,
  ...
  if ((len > THREAD_IO_CUTOFF) || (ba->flags & CAML_BA_MAPPED_FILE)) {
    ...
    written = write(Int_val(v_fd), bstr, len);
  if (written == -1)
    uerror("write_assume_fd_is_nonblocking", Nothing);
  return Val_long(written);
}
...
__pure static inline int contains_mmapped(value v_iovecs, int n) {
  for (--n; n >= 0; --n) {
    ...
    if (unlikely(flags & CAML_BA_MAPPED_FILE))
      return 1;
  }
  return 0;
}
...
CAMLprim value bigstring_writev_assume_fd_is_nonblocking_stub(value v_fd,
  ...
  if (ret == -1)
    uerror("writev_assume_fd_is_nonblocking", Nothing);
  return Val_long(ret);
}
...
CAMLprim value bigstring_recvmmsg_assume_fd_is_nonblocking_stub(
  ...
  /* On 32-bit platforms, sizeof(unsigned) == sizeof(intnat); it thus suffices
     to check that [v_count] is not negative.

     On 64-bit platforms with unsigned being 32 bit and intnat being 64 bit, we
     need the second check to ensure there is no truncation.  Note that
     "(intnat) count" zero-extends to 64-bit width.  This check actually
     subsumes the [v_count] being negative check.

     If this code were built on a platform where both unsigned and intnat were
     64 bit, then it should still work, by analogy with the all-32 bit case.
  */
