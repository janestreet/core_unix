...
let alloc n = ignore (List.init (exp2 n) ~f:(fun x -> x))
...
let () =
  ...
  let group_max, group_max_size = List.fold_left mx (0, 0) all_groups in
  let group_average, group_average_size =
    let measure (_, sz) = sz * (group_max_size - sz) in
    List.fold_left (fun x y -> if measure x > measure y then x else y) (0, 0) all_groups
  in
