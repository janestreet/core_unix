...
let preallocated_errnos : (unit, Unix_error.t) Result.t Iarray.t =
  Iarray.init 64 ~f:(fun i -> Error (Unix_error.of_errno i))
;;
...
(* Since we return [-errno] from C, we implicitly rely on there not being a 0 [errno].
   ...
   [errno]. *)
let%test "no 0 errno" =
  Poly.equal (Iarray.get preallocated_errnos 0) (Error (Unix_error.EUNKNOWNERR 0))
;;

let num_preallocated_errnos = Iarray.length preallocated_errnos
...
module%template [@mode m = (global, local)] Make (M : Arg [@mode m]) () = struct
  ...
  let preallocated_ms =
    let rec loop i rev_acc =
      (* Preallocate at most a handful of Ms. 2048 is the first round binary number after
         ...
         [Syscall_result.Int]. *)
      if i = 2048
      then Iarray.of_list_rev rev_acc
      else (
        match M.of_int_exn i with
        | exception _ -> Iarray.of_list_rev rev_acc
        | m -> loop (i + 1) ((Ok m : (_, Unix.error) result) :: rev_acc))
    in
    loop 0 []
  ;;

  let num_preallocated_ms = Iarray.length preallocated_ms
  ...
  let to_result t =
    ...
    then
      ...
      then Iarray.unsafe_get preallocated_ms t
      else Ok (M.of_int_exn t)
    else (
      let errno = -t in
      if errno < num_preallocated_errnos
      then (
        match Iarray.unsafe_get preallocated_errnos errno with
  ...
  module Private = struct
    let of_int t = t
    let length_preallocated_errnos = Iarray.length preallocated_errnos
    let length_preallocated_ms = Iarray.length preallocated_ms
  end
end
